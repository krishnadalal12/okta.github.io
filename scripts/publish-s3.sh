...
aws s3 sync --content-type 'text/html' --sse --dryrun --size-only --acl public-read dist/blog s3://developer.okta.com-production/blog

# Upload all other assets to S3
aws s3 sync --exclude 'blog' --sse --dryrun --size-only --acl public-read dist s3://developer.okta.com-production
